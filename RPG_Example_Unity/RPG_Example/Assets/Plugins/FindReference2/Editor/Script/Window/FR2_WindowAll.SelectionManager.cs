...
namespace vietlabs.fr2
{
    internal partial class FR2_WindowAll
    {
        ...
        private void OnSelectionManagerChanged()
        {
            ...
            if (hadPingLock)
            {
                FR2_LOG.Log("Skipped refresh: Ping lock was active - keeping current FR2 selection!");
            ...
            if (shouldRefresh)
            {
                ...
                {
                    FR2_LOG.Log("Cache not ready - selection updated but view refresh skipped");
                }
            }
            else
            {
            ...
            if (isFocusingGUIDs)
            {
                ...
                for (var i = 0; i < currentSelection.Length; i++)
                {
...
#if UNITY_2018_1_OR_NEWER
                    {
                        ...
                        {
                            FR2_LOG.LogWarning($"TryGetGUIDAndLocalFileIdentifier {item}\nException: {e}");
                        }
                    }
#else
                    {
        ...
        internal void SetFR2Selection(UnityObject[] objects)
        {
            ...
            {
                FR2_LOG.Log("Cache not ready - selection updated but view refresh skipped in SetFR2Selection");
            }
        }
        ...
        private void RefreshFR2View()
        {
            ...
            if (this is FR2_WindowAll window)
            {
            ...
            if (selection.isSelectingSceneObject)
            {
                ...
                foreach (string instIdStr in selection.instSet)
                {
                    ...
                    {
                        var obj = EditorUtility.EntityIdToObject(instId);
